...
draftdf = pd.read_csv('/Users/pauly/Desktop/Github/NBADraft/class_2023.csv')
# draftdf = pd.read_csv('class_2023.csv')
# player_names = []
...
def scrape_college_data(names=[]):
    stats = []
    for name in names:
        url = f"https://www.sports-reference.com/cbb/players/{name}.html"
        urlcleanup()
        html = urlopen(url)
        ...
        for row in soup_table.find_all('tr')[-1:]:# Excluding the first 'tr', since that's the table's title head
            player = {}
            player['Player']= (draftdf['Player'].loc[draftdf['FNAME'] == name]).item()
            ...
            player['WS/40'] = row.find('td', {'data-stat' : 'ws_per_40'}).text
            stats.append(player)
...
#Collecting data into dataframe then placing into a csv file
    ...
    print(df)
    df.to_csv('2023_college_statistics_ws40.csv')

scrape_college_data(draftdf['FNAME'])
# -
